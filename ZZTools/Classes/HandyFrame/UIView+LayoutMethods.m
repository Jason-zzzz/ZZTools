...
- (void)topInContainerRatio:(CGFloat)ratio shouldResize:(BOOL)shouldResize
{
    if (shouldResize) {
        self.ct_height = self.ct_y - self.superview.ct_height * ratio + self.ct_height;
    }
    self.ct_y = self.superview.ct_height * ratio;
}
...
- (void)fillWidth:(float)ratio
{
    self.ct_width = self.superview.ct_width * ratio;
    self.frame = CGRectMake((self.superview.ct_width - self.ct_width) / 2, self.ct_y, self.ct_width, self.ct_height);
}
...
- (void)fillHeight:(float)ratio
{
    self.ct_height = self.superview.ct_height * ratio;
    self.frame = CGRectMake(self.ct_width, (self.superview.ct_height - self.ct_y) / 2, self.ct_width, self.ct_height);
}
